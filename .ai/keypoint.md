...
- 启动计时时要捕获本次计时类型、时长、完成文案和声音策略；设置变化只影响下一次计时，不改变当前计时。
- 专注中和休息中都复用现有 `timer-waiting`，自然结束都复用现有巨型庆祝，不新增宠物状态或休息形象。
- 专注结束后不自动开始休息，休息结束后不自动开始下一轮专注，完成后直接回到未计时状态。
- 轻量专注计时实现中，`src-tauri/src/settings.rs` 负责 `FocusTimerPreferences` 默认值、旧设置兼容和 1-180/去重/文案规范化校验。
- 轻量专注计时实现中，`TimerState` 在启动时保存 kind、duration、ends_at、finished_message 和 play_sound，完成通知不再回读最新设置。
- 轻量专注计时菜单使用动态 ID：`start_focus_timer_{index}` 和 `start_break_timer`；计时中仍只更新已存在的状态菜单项文本。
